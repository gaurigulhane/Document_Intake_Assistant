...
This log records the development process, AI tool interactions, architectural decisions, and bug resolutions for the **Document Intake Assistant** technical test.

### Test Requirements Checklist:
- ✅ **Backend & User Interface**: Developed FastAPI backend and modern React 18 + Vite frontend UI.
- ✅ **Multi-Turn Conversation**: Supported multi-turn interactive dialogue with LangGraph state machine.
- ✅ **State Partitioning**: Maintained structured state (`WishesState`) strictly separate from conversation history.
- ✅ **Ambiguity & Conflict Handling**: Handled missing fields, unclear input (`needs_confirmation`), and contradictory input (interactive conflict modal).
- ✅ **AI Tools Leveraged**: ChatGPT (GPT-4o), Claude 3.5 Sonnet, GitHub Copilot, and Cursor.
- ✅ **Submission Assets**: Public GitHub repository with complete setup instructions, Pytest test suite, Docker containerization, AI log, and production improvements roadmap.

---

## 1. AI Tools & Assistants Used

- **ChatGPT (GPT-4o)**: Architecture brainstorming, Pydantic schema validation design, and prompt engineering strategy.
- **Claude 3.5 Sonnet**: Designing the LangGraph workflow state machine, conflict detection logic, and ReportLab PDF layout rules.
- **GitHub Copilot / Cursor**: Inline autocompletion, React hooks boilerplate, and FastAPI route structure.
...
- **Iteration Outcome**: Initial graph draft attempted to store state directly inside conversation messages. Refactored so that `StructuredStateModel` is strictly isolated as the source of truth, separate from raw message logs.
...
## 4. Key Engineering Decisions & Production Notes
...
- **Production Improvement Roadmap**:
  1. JWT / OAuth2 authentication for user session security.
  2. Transitioning SQLite to PostgreSQL with encrypted storage for sensitive state data.
  3. Pre-LLM input sanitization filters to prevent prompt-injection attacks.
